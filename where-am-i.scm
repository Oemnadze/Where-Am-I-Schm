...
(define (distance-product distance1 points1)
  (apply * (map (lambda (x) (dist distance1 x)) (remove distance1 points1))))

(define (rate-points lst)
  (apply append (map (lambda (x) (list (cons (distance-product x lst) (list x)))) lst)))

(define (sort-points lst)
  (quicksort lst))

(define (part-lst lst n)
  (if (= n 0) '()
    (append (list (car lst)) (part-lst (cdr lst) (- n 1)))))

(define (clumped-points lst)
  (let ((halfSorted (part-lst (sort-points (rate-points lst)) (quotient (length lst) 2))))
    (apply append (map (lambda (x) (cdr x)) halfSorted))))

(define (average lst)
  (/ (apply + lst) (length lst)))

(define (average-point lst)
  (let ((avg (list (average (map car lst)) (average (map cadr lst)))))
    (cons (distance-product avg lst) (list avg))))

;;(define (average-point lst)
;;  (let ((avg (pointsAvg lst)))
;;    (cons (distance-product avg lst) (list avg))))

(define (best-estimate lst)
  (average-point (clumped-points (intersection-points lst))))

(define (where-am-i distances1 points1)
  (let ((sorted (quicksort (apply append (map (lambda (x) (list (best-estimate x))) (all-guesses distances1 points1))))))
    (part-lst sorted (length sorted))))
